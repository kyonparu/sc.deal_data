...
%% 
    ...
    output_folder = sprintf('target_%d',target_date);
        %絶対パス(相対パスだと書き込み権限ダメだった)
        chT1_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit', edit_data_folder, output_root_folder, output_folder, ema_folder, sprintf('%d_T1.csv', file_num));
        chT2_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder, output_folder, ema_folder, sprintf('%d_T2.csv', file_num));
        chT3_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder, output_folder, ema_folder, sprintf('%d_T3.csv', file_num));
        chUL_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder, output_folder, ema_folder, sprintf('%d_UL.csv', file_num));
        chLL_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder, output_folder, ema_folder, sprintf('%d_LL.csv', file_num));
        chLJ_out_fullpath = fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder, output_folder, ema_folder, sprintf('%d_LJ.csv', file_num));

   %ないときは出力フォルダを作る
    if ~exist(fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder,output_folder, ema_folder), 'dir')
        mkdir(fullfile('D:\ikeda_data\DNN_exp\edit',edit_data_folder,output_root_folder,output_folder, ema_folder));
        sprintf('ディレクトリを作成')
    end   
